...
- On the URLPattern side, [whatwg/urlpattern#255](https://github.com/whatwg/urlpattern/pull/255) (rubycon, CONTRIBUTOR) proposed constructing a *properly defined* dummy URL directly (rather than parsing a placeholder string), defaulting its scheme to `https` for the "canonicalize a domain name" path specifically — non-special-scheme patterns and patterns with no `protocol` still route through the plain "canonicalize a hostname" path. annevk called the direction reasonable but noted the PR still left the dummy URL's construction not fully defined, and that its default may be wrong when `protocol` is absent — see the point above about no-protocol patterns needing the special-URL path too. The PR was closed unmerged; the issue remains open as of this writing, with the URLPattern-side fix still to be finalized on top of the now-merged url#863 groundwork.

## Other Participant Input

- **sisidovski (COLLABORATOR)**: independently spotted that the empty-string default scheme means a literal reading of the old spec wouldn't even reach the special-URL path, reinforcing that the spec was actually wrong, not just unclear. Iterated on the fix with annevk across several comments and volunteered to pick up the URLPattern-side work once url#863 landed.
